// -----------------------------------------------------------------------------------------------------------------
// <copyright file="MessageBus.cs" company="dwndland">
...
#pragma warning restore CS4014
    ...
    private void OnDisposed(object sender, EventArgs e)
    {
        ...
        foreach (var tuple in _subscribers)
        {
            if (!tuple.Value.Contains(sender))
                continue;

            tuple.Value.Remove(subscriber);
            break;
        }
    ...
    private void OnAsyncDisposed(object sender, EventArgs e)
    {
        ...
        foreach (var tuple in _asyncSubscribers)
        {
            if (!tuple.Value.Contains(sender))
                continue;
